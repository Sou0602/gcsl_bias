...
class GCSL:
    ...
    def __init__(self,
        ...
        self.m_policy_optimizer = torch.optim.Adam(self.m_policy.parameters() , lr = 5e-4)
        #pdb.set_trace()
    ...
    def loss_fn(self, observations, goals, actions, horizons, weights):
        ...
        marginal_nll = self.m_policy.nll(observations_torch, goals_torch, actions_torch,ref_pol = self.policy, horizon=horizons_torch)
        nll = conditional_nll + marginal_nll
        c_nll = conditional_nll
        m_nll = marginal_nll
    ...
    def sample_trajectory_buffer(self, greedy=False, noise=0, render=False):
        ...
        for t in range(self.max_path_length):
            ...
            #                                    noise=noise)[0]
            action = self.max_action(observation[None], goal[None], horizon=horizon[None], greedy=greedy,
                                                noise=noise)[0]
    ...
    def ratio_action(self, obs, goal, horizon=None, greedy=False, noise=0):
        ...
        logits = self.policy.net.forward(obs, goal, horizon=horizon)
        noisy_logits = logits * (1 - noise)
        probs_sg = torch.softmax(noisy_logits, 1)

        marginal_logits = self.m_policy.net.forward(obs, goal, horizon=horizon)
        eps = 0.00001
        probs_s = torch.softmax(marginal_logits, 1) + eps
        probs_s = probs_s / torch.sum(probs_s, dim=1)
    ...
    def max_action(self, obs, goal, horizon=None, greedy=False, noise=0):
        obs = torch.tensor(obs, dtype=torch.float32)
        goal = torch.tensor(goal, dtype=torch.float32)
        ...
        ent_logits = self.m_policy.net.forward(obs, goal, horizon=horizon)
        eps = 0.00001
        probs_s = torch.softmax(ent_logits, 1) + eps
        probs_s = probs_s / torch.sum(probs_s, dim=1)
        c_probs = 1 - probs_s
        probs = torch.maximum(probs_sg,c_probs)
        #probs = probs/torch.sum(probs, dim =1)
        if greedy:
            samples = torch.argmax(probs, dim=-1)
        else:
    ...
    def sample_trajectory(self, greedy=False, noise=0, render=False):
        ...
        for t in range(self.max_path_length):
            ...
            horizon = np.arange(self.max_path_length) >= (
                        self.max_path_length - 1 - t)  # Temperature encoding of horizon
            ## Have to update to take ratio action
            #action = self.policy.act_vectorized(observation[None], goal[None], horizon=horizon[None], greedy=greedy,
            #                                    noise=noise)[0]
            action = self.ratio_action(observation[None], goal[None], horizon=horizon[None], greedy=greedy,
                                       noise=noise)[0]
            if not self.is_discrete_action:
                action = np.clip(action, self.env.action_space.low, self.env.action_space.high)
            ...
            while total_timesteps < self.max_timesteps:
                ...
                if timesteps_since_eval >= self.eval_freq:
                    ...
                    if logger.get_snapshot_dir():
                        ...
                        torch.save(
                            self.m_policy.state_dict(),
                            osp.join(logger.get_snapshot_dir(), 'm_policy%s.pkl' % modifier)
                        )
                        ...
                        full_dict_m = dict(env=self.env, policy=self.m_policy)
                        with open(osp.join(logger.get_snapshot_dir(), 'm_params%s.pkl' % modifier), 'wb') as f:
                            pickle.dump(full_dict_m, f)
