from gcsl.algo import buffer, networks_off
...
def default_markov_policy(env, env_params):
    ...
    if env.action_space.n > 100: # Too large to maintain single action for each
        policy_class = networks_off.IndependentDiscretizedStochasticGoalPolicy
    else:
        policy_class = networks_off.DiscreteStochasticGoalPolicy
    return policy_class(
                env,
