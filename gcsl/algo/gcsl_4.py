...
class GCSL:
    ...
    def sample_trajectory_buffer(self, greedy=False, noise=0, render=False):
        ...
        if self.imbalanced_goals:
            ...
            g_avg = (g_low + g_high)/2
            g_avg_ = (g_high - g_low)/2
            '''''
            g_l = np.random.rand(4)*g_avg_ + g_low
            g_r = -np.random.rand(4)*g_avg_ + g_high
            #print('g_l',g_l)
            #print('g_r', g_r)
            ...
            if left:
                #goal[0] = g_avg[0]
                #goal[1] = g_avg[1]
                goal[2] = g_low[2] + (g_avg_[2]) * np.random.rand()
                goal[3] = g_low[3]
                goal[0] = goal[2]
                goal[1] = goal[3]
            else:
                #goal[0] = g_avg[0]
                #goal[1] = g_avg[1]
                goal[2] = g_high[2] - (g_avg_[2])  * np.random.rand()
                goal[3] = g_low[3]
                goal[0] = goal[2]
                goal[1] = goal[3]
            # print('goal_0', goal[0])
