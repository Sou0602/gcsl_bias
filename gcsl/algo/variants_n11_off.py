from gcsl.algo import buffer, networks_n11_off
...
def default_markov_policy(env, env_params):
    ...
    if env.action_space.n > 100: # Too large to maintain single action for each
        policy_class = networks_n11_off.IndependentDiscretizedStochasticGoalPolicy
    else:
        policy_class = networks_n11_off.DiscreteStochasticGoalPolicy_m
    return policy_class(
                env,
...
def default_markov_policy_buffer(env):
    assert isinstance(env.action_space, Discrete)
    if env.action_space.n > 100: # Too large to maintain single action for each
        policy_class = networks_n11_off.IndependentDiscretizedStochasticGoalPolicy
    else:
        policy_class = networks_n11_off.DiscreteStochasticGoalPolicy
    return policy_class(
                env,
                state_embedding=None,
                goal_embedding=None,
