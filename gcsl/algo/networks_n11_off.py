...
class DiscreteStochasticGoalPolicy(nn.Module, policy.GoalConditionedPolicy):
    ...
    def act_vectorized_bias(self, obs, goal, horizon=None, greedy=False, noise=0,
        ...
        if np.random.rand(1) < 0.05:
            prob_bias = probs
            return ptu.to_numpy(torch.argmax(prob_bias,dim=-1))
        else:
            ind_max = torch.argmax(probs)
            #pdb.set_trace()
            prob_bias = torch.cat([probs[: , 0:ind_max],probs[: , ind_max+1:]],axis = 1)
            prob_bias = prob_bias/torch.sum(prob_bias , axis = 1 )
        #pdb.set_trace()
        if greedy:
            samples = torch.argmax(prob_bias, dim=-1)
        else:
            samples = torch.distributions.categorical.Categorical(probs=prob_bias).sample()
        #pdb.set_trace()
