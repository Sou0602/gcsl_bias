from gcsl.algo import buffer, networks_ne
...
def get_params(env, env_params):
    ...
    return env, policy,m_policy, replay_buffer, gcsl_kwargs
...
def default_markov_policy_me(env, env_params):
    ...
    if env.action_space.n > 100: # Too large to maintain single action for each
        ...
        policy_class = networks_ne.DiscreteStochasticGoalPolicy_me
    return policy_class(
                env,
...
def default_markov_policy(env, env_params):
    assert isinstance(env.action_space, Discrete)
    if env.action_space.n > 100: # Too large to maintain single action for each
        policy_class = networks_ne.IndependentDiscretizedStochasticGoalPolicy
    else:
        policy_class = networks_ne.DiscreteStochasticGoalPolicy
    return policy_class(
                env,
                state_embedding=None,
                goal_embedding=None,
