    ...
    def sample_trajectory_buffer(self, greedy=False, noise=0, render=False):
        ...
        if self.imbalanced_goals:
            ...
            if left:
                # print('left')
                self.goal_side = -1
                goal[0] = g0_low + 0.001 * np.random.rand()
                goal[1] = g1_low + 0.001 * np.random.rand()
                goal[2] = g2_low + 0.001 * np.random.rand()
                goal[3] = g3_low + 0.001 * np.random.rand()
            else:
                # print('right')
                self.goal_side = 1
                goal[0] = g0_high - 0.001 * np.random.rand()
                goal[1] = g1_high - 0.001 * np.random.rand()
                goal[2] = g2_high + 0.001 * np.random.rand()
                goal[3] = g3_high + 0.001 * np.random.rand()
            '''''
                ...
                if self.is_offline:
                    if self.replay_buffer.current_buffer_size < self.replay_buffer.max_buffer_size:
                        if total_timesteps < self.explore_timesteps:
                            ...
                            states, actions, goal_state = self.sample_trajectory_buffer(greedy=False,
