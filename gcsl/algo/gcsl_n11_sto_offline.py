...
from gcsl.algo import variants_n11_off
...
class GCSL:
    ...
    def __init__(self,
        ...
        self.validation_buffer.max_buffer_size = 20000
    ...
    def sample_trajectory_buffer(self, policy_buffer , greedy=False, noise=0, render=False):
        ...
        for t in range(self.max_path_length):
            ...
            action = policy_buffer.act_vectorized_bias(observation[None], goal[None], horizon=horizon[None], greedy=False,
                                                noise=noise)[0]
    ...
    def offline_buffer(self):
        b_size = self.replay_buffer.max_buffer_size
        model = variants_n11_off.default_markov_policy_buffer(self.env)
        state_dict = torch.load(self.model_path)
        model.load_state_dict(state_dict)
        model.eval()

        for _ in range(int(self.explore_timesteps/10)):
            states, actions, goal_state = self.sample_trajectory_buffer(model,noise=1)
        ...
        for _ in range(b_size):
            states, actions, goal_state = self.sample_trajectory_buffer(model,greedy=False, noise=self.expl_noise)
            self.replay_buffer.add_trajectory(states,actions,goal_state)

        for _ in range(self.validation_buffer.max_buffer_size):
            states, actions, goal_state = self.sample_trajectory_buffer(model, greedy=False, noise=self.expl_noise)
            self.validation_buffer.add_trajectory(states, actions, goal_state)
    ...
    def train(self):
        ...
        with tqdm.tqdm(total=self.eval_freq, smoothing=0) as ranger:
            while total_timesteps < self.max_timesteps:
                if self.replay_buffer.current_buffer_size < b_size:
